...
export default function VideosContents() {
  ...
  const paginatedData = useMemo(() => {
    ...
    return {
      ...
      endIndex: Math.min(endIndex, videos.length),
    };
  }, [videos, currentPage]);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const newTotalPages = Math.ceil(updatedVideos.length / ITEMS_PER_PAGE);
  ...
  const handlePlayVideo = async (videoId: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      setError(
        "Failed to play video. Please check if the video URL is accessible.",
      );
    }
  };
    ...
    <div className="relative h-screen flex flex-col">
        ...
        <div className="px-4 py-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-gray-500 mt-0.5">
              ...
              {paginatedData.totalItems} videos
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="flex-1 p-3 overflow-hidden">
        <div className="grid grid-cols-5 gap-2.5 h-full">
          {paginatedData.paginatedVideos.map((video) => (
            <div
              key={video.id}
              ...
              <div className="relative aspect-square bg-gray-100 shrink-0 overflow-hidden">
                {videoErrors[video.id] ? (
                  ...
                  <>
                    ...
                    {playingVideoId !== video.id && !videoLoading[video.id] && (
                      <div
                        onClick={(e) => handlePlayVideo(video.id, e)}
