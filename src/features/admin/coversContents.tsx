...
export default function CoversContents() {
  ...
  const paginatedData = useMemo(() => {
    ...
    const paginatedCovers = covers.slice(startIndex, endIndex);
    ...
    return {
      ...
      endIndex: Math.min(endIndex, covers.length),
    };
  }, [covers, currentPage]);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const newTotalPages = Math.ceil(updatedCovers.length / ITEMS_PER_PAGE);
    ...
    <div className="relative h-screen flex flex-col">
        ...
        <div className="px-4 py-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-gray-500 mt-0.5">
              ...
              {paginatedData.totalItems} covers
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="flex-1 p-3 overflow-hidden">
        <div className="grid grid-cols-5 gap-2.5 h-full">
          {paginatedData.paginatedCovers.map((cover) => (
            <div
              key={cover.id}
